...
- 采用模块化设计，信息源、采集器、筛选阈值、选题规则和渲染模板都可以替换；更换新闻源和筛选标准后，也可以复用为金融、科技、教育、出海、电商等其他领域的新闻日报系统。
...
这种拆分方式让项目可以按领域迁移：在 `src/config/sourceRegistry.ts` 替换信息源，在筛选和选题配置中调整相关性、新闻价值和分类配额，再按目标平台改渲染模板，就能生成其他行业的日报或周报。
